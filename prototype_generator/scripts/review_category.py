...
BAD_WORDS = ("diagram", "map", "plan", "poster", "kit", "logo", "chart", "graph", "schematic", "icon", "sign", "flag", "book")


def api_category_members(category, limit=25):
    params = {
        "action": "query",
...
def main():
    ...
    data = api_category_members(category)
    cands = candidates(data, n)
    if not cands:
        print(f"No candidates for {key} (Category:{category})")
        return
    for i, c in enumerate(cands, 1):
        dest = os.path.join(REVIEW_DIR, f"{key}_{i}.jpg")
        try:
            download(c["thumburl"], dest)
            print(f"{key}_{i}.jpg <- {c['title']}")
