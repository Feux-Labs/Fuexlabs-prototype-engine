...
// cycles through a small tonal palette derived from the page's own accent
// color, the whole ribbon slowly swirls and drifts through a hue-rotate
// cycle, and a handful of the brightest lines carry a glowing dot that
// travels the full curve on a loop (pure SMIL, no client JS). Deterministic
// pseudo-randomness (Math.sin, not Math.random) so server and client render
// identically.
const LINE_COUNT = 46;
const VB_W = 1000;
const VB_H = 620;
...
export default function GlowRibbon({ accent, accentDark }: { accent: string; accentDark: string }) {
  const palette = [accent, mixHex(accent, "#ffffff", 0.35), accentDark, mixHex(accentDark, "#000000", 0.3)];
    ...
    <svg className="ribbon-bg" viewBox={`0 0 ${VB_W} ${VB_H}`} preserveAspectRatio="xMidYMid slice" aria-hidden="true">
      ...
      <g className="ribbon-swirl">
        <g fill="none" filter="url(#ribbon-glow)">
          {LINES.map((l, i) => (
            <path key={i} d={l.d} stroke={lineColor(i)} strokeWidth={l.width} opacity={l.opacity} />
          ))}
        </g>

        {SPARK_INDICES.map((idx, k) => (
          <circle key={idx} r="3.5" fill={lineColor(idx)} filter="url(#ribbon-spark-glow)">
